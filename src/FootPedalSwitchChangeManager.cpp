...
void FootPedalSwitchChangeManager::DoSomething(int buttonIndex)
{
...
// #else
//   DBG_PRINT_LN("FootPedalSwitchChangeManager::SendCurrentProgramNumberChange() - Sending Program Change = " + String(mCurMidiProgramNum) + ".");
// #endif
//   gStatusManager.OnMidiEvent(MidiEventType::Other, mCurMidiProgramNum, zeroBasedMidiChannel);

// TODO: Send Yamaha SX-700/900 SysEx based on which switch is pressed.
#ifdef SEND_MIDI
  // TODO: Send SysEx
  ...
  const unsigned char SwitchOff = 0x00; // TBD: What is Switch On/Off?
	...
	Serial.write(SwitchOn); // Switch On/Off.
  Serial.write(0xF7); // End of Exclusive
#else
  DBG_PRINT_LN("FootPedalSwitchChangeManager::DoSomething() - buttonIndex = " + String(buttonIndex) + ".");
#endif
  // TODO: gStatusManager.OnMidiEvent(MidiEventType::Other, mCurMidiProgramNum, zeroBasedMidiChannel);
}